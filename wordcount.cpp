...
{
	string testWords[] = {"This","is","n0t","the","1st","test","or","is","it","test","test","test","test"};//{"test1", "sadfsa", "coolsd", "23adfaw", "sdfa", "a","f43", "coolsd","pardon","double","criticize","supervision","guilt","deck","payable","execution","suite","elected","solel","moral","collector","questionnaire","flavour","couple","faculty","tour","basket","mention","kick","horion","drain","happiness","fighter","estimated","copper","legend","relevance","decorate","continental","shi","operational","incur","parallel","divorce","opposed","equilibrium","trader"};
	HashMap hmap;

	for (int i = 0; i < 14; ++i) {
		string word = testWords[i];
		hmap.increment(word);
		cout << word << " -> " <<  hmap.get(word) << endl;
	}
	cout << "Done" << endl << endl;;

	for (HashMap::iterator entryp = hmap.begin(); entryp < hmap.end(); entryp++) {
		if (*entryp) // only look at slots with (*entryp) in them
			cout << (*entryp)->getKey() << " - > " << (*entryp)->getValue() << endl;
	}
